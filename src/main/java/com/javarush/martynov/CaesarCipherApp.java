...
import java.util.*;
    ...
    public class CaesarCipherApp {
        ...
        private static void decryptMode(Scanner scanner) {
            System.out.println("\n=== Расшифровка файла по ключу ===");
            File inputFile = getInputFile(scanner);
            if (inputFile == null) return;
        ...
        private static File getOutputFile(Scanner scanner) {
        ...
        private static void processFile(File inputFile, File outputFile, int key, boolean encrypt) throws IOException {
            // Буфер для чтения
        ...
        private static final Map<Character, Double> RUSSIAN_FREQ = createRussianFreqMap();

        private static Map<Character, Double> createRussianFreqMap() {
         ...
         */
        private static double frequencyScore(Map<Character, Double> freqText, Map<Character, Double> freqRussian) {
            double score = 0.0;
            for (char c = 'А'; c <= 'Я'; c++) {
         ...
         */
        private static List<Integer> statisticalAttack(String ciphertext) {
            int n = ALPHABET.length();
